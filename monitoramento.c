...
int main() {
    ...
    do {
        limite_temperatura = lerEntrada_float("Informe o limite de temperatura de segurança (-50 a 150 C): ");

        if (limite_temperatura < -50.0 || limite_temperatura > 150.0) {
            printf("Error: O limite de segurança deve estar entre -50 C e 150 C.\n");
        }
    } while (limite_temperatura < -50.0 || limite_temperatura > 150.0);

    printf("\nLimite seguro configurado para: %.2f C\n", limite_temperatura);
    printf("Iniciando... (Encerrara com 3 alertas consecutivos)\n");
    printf("-----------------------------------------------------\n");


    while (tentativas_consecutivas < 3) {
        temp_atual = lerEntrada_float("Digite a temperatura atual: ");
        ...
        if (temp_atual > limite_temperatura) {
            ...
            printf("[AVISO] Temperatura dentro do limite seguro.\n");
        }
    }
